...
| agreement_type | string | **Yes** | Sign type: `CYCLE` (periodic) / `NON_CYCLE` (non-periodic) / `SINGLE` (one-time) |
| merchant_user_id | string | **Yes** | Merchant-side user ID (for mapping) |
| scene_code | string | **Yes** | [Scene code](appendix#scene-codes): TAXI, PARKING, SUBSCRIPTION, etc. |
| product_code | string | No | Product code (assigned by platform) |
...
| extra_params | object | No | Extension parameters (passed through) |
...
| data.qr_code | string | Sign QR code content (for App scan) |
| data.qr_code_url | string | Sign QR code image URL |
| data.expire_time | string | Sign link expiration time |
